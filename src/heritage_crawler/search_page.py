...
**この見出しは全 19 分類で同じ** (2026-08-23 実測)。CSV の方は 12 列目の見出しが
分類で変わる (`ledger.AREA_COLUMN_LABELS`) が、一覧は 1 つの表で全分類を出すので
分岐が要らない。ただし**中身は分類で変わる** — 美術工芸品は
``北海道（北海道）`` (所有者住所と所在都道府県)、無形文化財は空。
"""
