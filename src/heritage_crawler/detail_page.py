...
class DetailPage:
    ...
    ``附指定`` / ``指定等後に行った措置`` / ``添付ファイル`` のほか、分類によって
    ``附`` (301 / 311)・``一つ書`` (201 / 211)・保持者や保持団体の見出し
    (303 / 323 / 313 / 304) が現れる。**901 では他分類の名前そのもの**が並ぶ
    (構成資産へのリンク)。意味付けは ``record`` に任せる。
    """
    ...
    **何の一覧かは分類で変わる。** 建造物系は附指定 (``附名称`` / ``附員数``)、
    ...
    無形文化財は保持者・保持団体 (``保持者（関係技芸者）の氏名`` ほか)。
