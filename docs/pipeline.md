...
| ジョブ全体 | 失敗したら Issue が立つ (同じ Issue が open なら追記)。本文は**押す前に落ちたか、押したあとか**で書き分ける — データが更新済みかどうかが変わる |

### 3. 配る前 (heritages の `deliver.yml`)

`heritage-site build --check-only` が 9 点を見る (実装は heritages の
...
| 座標の欠けが 5% を超えていないか | 超えるのは台帳の取得漏れを疑う水準。それ以下なら**地図に出さないだけ**で一覧には出す | error / それ以下は報告のみ |
