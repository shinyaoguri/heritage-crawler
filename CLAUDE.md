...
- **1 req/s を守っていても 504 が返る時間帯がある。** 2026-08-12 の 23:47 JST に
  ...
  再試行 (2 秒・4 秒) では抜けられないため、**月次は `scripts/retry.sh` で
...
要件議論の主要な決定は ADR 0001〜0019 に記録済み。実装は 2 段構えの取得層
...
棟単位で単位が違うため件数表示の差で見る)。**月次更新 (#10) は完了**
— Actions からの疎通・差分検出の方法 (ADR 0018)・差分の突き合わせと出力のマージ
(`update-records`)・月次ワークフロー (`.github/workflows/monthly.yml`)・push 用の
GitHub App の secret まで入っていて、dry-run が通っている。**push まで含む本番は
2026-09-02 03:00 JST の定期実行が初回**で、そこまで確かめられていない。
ロードマップと残る論点は Issue #1 を正本とする。

**差分更新は「前回の状態」をデータリポジトリの JSON Lines から読む** (ADR 0018)。
キャッシュも生 HTML も Actions へ持ち回らない (Actions のキャッシュは 7 日で
エビクトされ月次には持たない)。取り直すのは新規・台帳の値が変わったぶん・
**毎月 1/12 の巡回**だけ — ソース側に更新日が無いため、詳細ページだけの項目
(解説文・員数など) の変更は取り直して比べるしか捕まえられない。**スキーマ変更時の
全件再組み立てはローカル**に残す (Actions は差分専用)。

### 差分更新の作り (#10)

- **台帳と比べてよい列は実測で決めた** (#10 のコメント)。出力に入っているのは
  詳細ページ由来の値なので (CSV から採るのは緯度経度だけ)、一致するとは限らない。
  23,742 件で一致率を測り、99.9% 以上だった列だけを `update.LEDGER_FIELDS` に置いた
- **102 の `種別1` は種別ではなく国宝・重文区分** (一致率 0%)。`CATEGORY_FIELDS` で
  読み替える。**都道府県は比較に使えない** (出力側は正規化済みで `２県以上` は値を
  持たない)。所在地の比較で足りる
- **CSV 側が空の欄は比べない。** 一覧から回収した行 (ADR 0017) は所在地も種別も
  持たず、比べると毎月「変わった」ことになる
- 巡回の割り当ては `sha256((台帳ID, 管理対象ID))` を 12 で割った余り。**組み込みの
  `hash` は実行ごとに種が変わる**ので使えない (毎月ちがう 1/12 を取ってしまう)
...
- 表示名は前回の `meta.json` を土台に、今月組み立てたぶんで上書きする。毎月 2,000 件
  しか組み立てないので、今月ぶんだけで作ると `meta.json` が月ごとに揺れる
- 手元の全件で**詳細キャッシュ無しのマージが前回の出力と 1 バイトも変わらない**ことを
  確かめてある (415 ファイル)。生成物の決定性はここで実証している
- **データが変わらない月も 10 リポジトリにコミットが 1 つずつ立つ。** `meta.json` の
  利用日が実行日だから (ADR 0018)。実測でも、月だけ進めると JSON Lines 405 ファイルは
  不動で `meta.json` 10 個の `accessed_date` だけが動いた。利用日は「実際にアクセス
  した日」なので動くのが正しく、**コミットメッセージで「確認しただけの月」と区別する**
  (`... に確認 (データの変更なし)` / `... の差分を反映`)
...
関与しない** — 月次でデータリポジトリへ push すれば、翌日に heritages 側が全部入りと
...
- `.github/workflows/monthly.yml` — 月次の差分更新 (取得そのものが目的)。
  毎月 2 日 03:00 JST。push には code4heritage org の GitHub App が要る
