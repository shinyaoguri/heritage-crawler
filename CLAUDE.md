...
棟単位で単位が違うため件数表示の差で見る)。月次更新 (Issue #10) は
Actions からの疎通・差分検出の方法 (ADR 0018)・差分の突き合わせと出力のマージ
(`update-records`)・月次ワークフロー (`.github/workflows/monthly.yml`) まで
入っている。**残るのは push 用の GitHub App の登録だけ** (secret が無いと
初回の実行が最初のステップで止まる)。
...
- **台帳と比べてよい列は実測で決めた** (Issue #10 のコメント)。出力に入っているのは
