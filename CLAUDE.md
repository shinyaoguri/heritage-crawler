...
棟単位で単位が違うため件数表示の差で見る)。残るのは月次更新の Issue #10 で、
Actions からの疎通・差分検出の方法 (ADR 0018)・**差分の突き合わせと出力のマージ
(`update-records`) までは入っている。残るのは月次ワークフロー**
(clone → 実行 → commit / push、失敗時の Issue 起票)。
...
例外は `.github/workflows/reachability.yml` の疎通確認だけで、これは
`workflow_dispatch` のみ (押したときしか走らない)。月次更新 (#10) を Actions で
回す前提が成り立つか — データセンター IP から取れるか — を、月次更新が使う
3 経路 (台帳の CSV・検索結果一覧・詳細ページ) で確かめる。既定は 126 件の 103。
