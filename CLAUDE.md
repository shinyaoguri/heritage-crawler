...
リポジトリ 11 個を使い、新しくは作らない (**うち 10 個が埋まる**。残る
`preservation-districts-for-groups-of-traditional-buildings` は、国が選定するのが
**重要**伝統的建造物群保存地区だけでデータベースに対応するデータが無く、空のまま)。
...
要件議論の主要な決定は ADR 0001〜0012 に記録済み。実装は 2 段構えの取得層
