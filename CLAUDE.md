...
- **棟単位の取得対象は 20,461 件** (2026-08-11 に台帳を全件取得して確定。
  ...
  4 req/s で約 1.4 時間、生 HTML は gzip 圧縮で約 200 MB になる (ADR 0010)
- 公共サイトが相手なのでレートに上限を設け、User-Agent に連絡先を記載する (ADR 0010)。
  `Accept-Encoding: gzip` も送るが、**サーバは gzip を返さない** (実測)
...
要件議論の主要な決定は ADR 0001〜0010 に記録済み。実装は 2 段構えの取得層
